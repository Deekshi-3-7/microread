...
function calculateCurrentStreak(entries: ReadingEntry[]): number {
...
function calculateLongestStreak(entries: ReadingEntry[]): number {
  const uniqueDates = Array.from(
    new Set(entries.map((entry) => entry.reading_date))
  ).sort()
...
export default function Insights() {
  ...
  async function loadInsights() {
    ...
    try {
      ...
      const readingEntries = (entriesData ?? []) as ReadingEntry[]
  ...
  const averagePages =
    readingDays > 0 ? Math.round(totalPages / readingDays) : 0

  const averageMinutes =
    readingDays > 0 ? Math.round(totalMinutes / readingDays) : 0
  ...
  entries.forEach((entry) => {
    const month = entry.reading_date.substring(0, 7)

    const existing = monthlyMap.get(month)

    if (existing) {
      existing.pages += Number(entry.pages_read || 0)
      existing.minutes += Number(entry.minutes || 0)
      existing.days = new Set(
        entries
          .filter(
            (item) => item.reading_date.substring(0, 7) === month
          )
          .map((item) => item.reading_date)
      ).size
    } else {
      monthlyMap.set(month, {
        month,
        pages: Number(entry.pages_read || 0),
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-gray-600">
            Understand your reading journey and consistency.
          </p>
        </div>

        <div className="rounded-xl border border-gray-200 bg-white p-8 text-center">
          <p className="text-gray-500">
            Loading your reading insights...
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <p className="text-sm text-gray-500">Reading Days</p>
          <p className="mt-2 text-3xl font-bold text-gray-900">
            {readingDays}
          </p>
          <p className="mt-1 text-xs text-gray-500">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        {currentBook ? (
          <div className="mt-5">
            ...
            <div className="mt-4 h-3 overflow-hidden rounded-full bg-gray-100">
              <div
                ...
                style={{
                  width: `${Math.min(currentBookProgress, 100)}%`,
                }}
              />
            </div>
          ...
          <p className="mt-4 text-sm text-gray-500">
            You don't have a book currently in progress.
          </p>
        )}
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        {monthlyStats.length === 0 ? (
          <p className="mt-4 text-sm text-gray-500">
            Your monthly reading summary will appear here once you
            start logging reading sessions.
          </p>
        ) : (
            ...
            <table className="w-full min-w-[600px] text-left">
              <thead>
                <tr className="border-b border-gray-200 text-sm text-gray-500">
                  ...
                  <th className="pb-3 font-medium">Reading Days</th>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-gray-600">
          The goal isn't to read as much as possible in one day.
          It's to keep showing up. Small pages, repeated consistently,
